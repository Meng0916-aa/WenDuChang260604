...
(process design), `configs/xtherm_format.yaml` (XTherm binary format and
temperature validity), `configs/physical_calibration.yaml` (spatial / geometry /
frame rate / process metadata), `configs/roi_strategy.yaml`, and
`configs/thermal_feature_contract.yaml` by **relative path only**. `configs/default.yaml` is
...
## Authoritative configuration chain
...
`configs/xtherm_format.yaml` owns the XTherm binary layout, raw-count scaling,
camera-valid temperature range, above-range/hard-saturation rules, and
conversion-QC thresholds. `configs/physical_calibration.yaml` owns spatial
scale, frame rate, image geometry, and process/optical physical metadata. Any
temperature binary fields retained in physical calibration are deprecated
compatibility mirrors and must match `configs/xtherm_format.yaml`.
...
| `configs/xtherm_format.yaml` | XTherm binary layout, scaling, temperature validity, QC thresholds | yes |
| `configs/physical_calibration.yaml` | spatial / geometry / fps / optical and process physical metadata | yes (only pixel-size source) |
